...
class OneClickDemoImport {
  ...
  private $plugin_page_setup = array();
	...
	public function import_demo_data_ajax_callback() {
         ...
         * 'full' : import full
         */
        $user_action = isset( $_POST[ 'import' ] ) ? $_POST[ 'import' ] : '';
		...
		if ( ! $use_existing_importer_data ) {
			...
			$this->selected_index = empty( $_POST['selected'] ) ? 0 : $_POST['selected'];
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'wi' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'wi' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'wi' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'wi' ) );
			}
		}
         ...
         * @since Fox 4.4
         */
        if ( 'uninstall' == $user_action ) {
            
            $slug = $this->selected_index;

            if ( ! isset( $this->import_files[ $slug ] ) ) {
                wp_send_json( array( 'status' => 'error' ) );
                return;
            }

            // 01 - delete posts and media
            $query_args = [
                'posts_per_page' => -1,
                'post_status' => 'any',
                'post_type' => 'any',
            ...
            // 02 - delete sidebars (and widgets)
            // $sidebars = isset( $this->import_files[ $slug ][ 'sidebars' ] );
            
            // 03 - customizer?

            // Send a JSON response with final report.
            $this->final_response( 'Demo uninstalled successfully' );
         ...
         * @withemes: only import content if full
		 */
        if ( 'full' === $_POST[ 'import' ] ) {
	 ...
	 * Send a JSON response with final report.
	 */
	private function final_response( $msg = '' ) {
		...
		if ( empty( $this->frontend_error_messages ) ) {
            ...
            if ( ! $msg ) {
                $msg = 'The demo import has finished. Please check your page and make sure that everything has imported correctly.';
            }

			$response['message'] .= '<div class="fox-notice notice-success"><p>' . $msg . '</p></div>';
		}
		else {
			$response['message'] = $this->frontend_error_messages_display() . '<br>';
			$response['message'] .= '<div class="notice notice-warning"><p>The demo import has finished, but there were some import errors. More details about the errors can be found in this <a href="' . Helpers::get_log_url( $this->log_file_path ) . '">log file</a></p></div>';
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'wi', false, plugin_basename( dirname( dirname( __FILE__ ) ) ) . '/languages' );
	}
